...
class AMQP::Client
  ...
  class Queue
    ...
    def unbind(exchange : String, routing_key : String, arguments = Arguments.new)
      @channel.queue_unbind(@name, exchange, routing_key, arguments)
      self
    end

    # Publish a message directly to the queue
    def publish(message, mandatory = false, immediate = false, properties = Properties.new)
      @channel.basic_publish(message, "", @name, mandatory, immediate, properties)
    end

    # Publish a message directly to the queue
    def publish(io : IO, bytesize : Int, mandatory = false, immediate = false, properties = Properties.new)
      @channel.basic_publish(io, bytesize, "", @name, mandatory, immediate, properties)
    end

    # Publish and confirm a message directly to the queue
    def publish_confirm(message, mandatory = false, immediate = false, properties = Properties.new)
      @channel.basic_publish_confirm(message, "", @name, mandatory, immediate, properties)
    end

    # Publish and confirm a message directly to the queue
    def publish_confirm(io : IO, bytesize : Int, mandatory = false, immediate = false, properties = Properties.new)
